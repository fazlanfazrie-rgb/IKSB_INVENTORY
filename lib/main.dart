...
class TranXStoreApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      ...
      theme: ThemeData(useMaterial3: true, colorScheme: scheme, scaffoldBackgroundColor: _forest),
...
class _DashboardPageState extends State<DashboardPage> {
  ...
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(title: const Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text('TranX_Store', style: TextStyle(fontWeight: FontWeight.w800)),
      Text('PLANTATION INVENTORY', style: TextStyle(fontSize: 10, letterSpacing: 1.4, color: _lime)),
    ]), actions: const [Padding(padding: EdgeInsets.only(right: 14), child: Icon(Icons.eco_outlined))]),
    body: _pages[_index],
...
class _DashboardView extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => ListView(padding: const EdgeInsets.fromLTRB(16, 10, 16, 24), children: [
...
class _InventoryView extends StatefulWidget {
  const _InventoryView(); @override State<_InventoryView> createState() => _InventoryViewState();
}
class _InventoryViewState extends State<_InventoryView> {
  ...
  @override Widget build(BuildContext context) => FutureBuilder<List<Map<String, Object?>>>(future: _future, builder: (context, snapshot) {
    ...
    return ListView(padding: const EdgeInsets.all(16), children: [
      const Text('Inventory', style: TextStyle(fontSize: 25, fontWeight: FontWeight.w800)), const SizedBox(height: 12),
      const TextField(decoration: InputDecoration(prefixIcon: Icon(Icons.search), hintText: 'Search item code or name', border: OutlineInputBorder())), const SizedBox(height: 12),
      ...
      else ...rows.map((r) => Card(child: ListTile(leading: const Icon(Icons.inventory_2, color: _lime), title: Text('${r['item_code']} • ${r['item_name']}'), subtitle: Text('${r['uom']}'), trailing: Text('${r['balance']}', style: const TextStyle(fontWeight: FontWeight.bold))))),
    ]);
  });
}

class _TransactionView extends StatefulWidget {
  const _TransactionView({required this.type}); final String type; @override State<_TransactionView> createState() => _TransactionViewState();
}
class _TransactionViewState extends State<_TransactionView> {
  final _item = TextEditingController(), _qty = TextEditingController(), _doc = TextEditingController(), _charging = TextEditingController(), _remark = TextEditingController();
  String _message = '';
  ...
  @override Widget build(BuildContext context) {
    ...
    return ListView(padding: const EdgeInsets.all(16), children: [
      Text(isReceive ? 'Receive Stock' : 'Issue Stock', style: const TextStyle(fontSize: 25, fontWeight: FontWeight.w800)),
      const SizedBox(height: 6), Text(isReceive ? 'Record goods received into the estate store.' : 'Record stock issued to operations.'), const SizedBox(height: 18),
      TextField(controller: _item, decoration: const InputDecoration(labelText: 'Item Code', prefixIcon: Icon(Icons.inventory_2), border: OutlineInputBorder())), const SizedBox(height: 12),
      TextField(controller: _qty, keyboardType: const TextInputType.numberWithOptions(decimal: true), decoration: InputDecoration(labelText: isReceive ? 'Quantity Receive' : 'Quantity Issue', prefixIcon: const Icon(Icons.scale), border: const OutlineInputBorder())), const SizedBox(height: 12),
      TextField(controller: _doc, decoration: const InputDecoration(labelText: 'Document No', prefixIcon: Icon(Icons.description_outlined), border: OutlineInputBorder())), const SizedBox(height: 12),
      TextField(controller: _charging, decoration: const InputDecoration(labelText: 'Charging / Location', prefixIcon: Icon(Icons.place_outlined), border: OutlineInputBorder())), const SizedBox(height: 12),
      TextField(controller: _remark, maxLines: 3, decoration: const InputDecoration(labelText: 'Remark', border: OutlineInputBorder())), const SizedBox(height: 16),
      FilledButton.icon(onPressed: _save, icon: const Icon(Icons.save), label: Text('SAVE ${isReceive ? 'RECEIVE' : 'ISSUE'}')),
      if (_message.isNotEmpty) ...[const SizedBox(height: 12), Card(child: Padding(padding: const EdgeInsets.all(14), child: Text(_message)))],
    ]);
  }
}

class _MoreView extends StatelessWidget {
  const _MoreView(); void _open(BuildContext c, Widget p) => Navigator.of(c).push(MaterialPageRoute(builder: (_) => p));
  @override Widget build(BuildContext context) => ListView(children: [
    const Padding(padding: EdgeInsets.fromLTRB(16, 18, 16, 8), child: Text('Modules', style: TextStyle(fontSize: 25, fontWeight: FontWeight.w800))),
    ListTile(leading: const Icon(Icons.receipt_long, color: _lime), title: const Text('Bin Card'), trailing: const Icon(Icons.chevron_right), onTap: () => _open(context, const BinCardPage())),
    ListTile(leading: const Icon(Icons.fact_check, color: _lime), title: const Text('Stock Take'), trailing: const Icon(Icons.chevron_right), onTap: () => _open(context, const StockTakePage())),
    ListTile(leading: const Icon(Icons.local_gas_station, color: _lime), title: const Text('Fuel'), trailing: const Icon(Icons.chevron_right), onTap: () => _open(context, const FuelPage())),
    ListTile(leading: const Icon(Icons.eco, color: _lime), title: const Text('Fertilizer'), trailing: const Icon(Icons.chevron_right), onTap: () => _open(context, const FertilizerPage())),
    ListTile(leading: const Icon(Icons.assessment, color: _lime), title: const Text('Reports'), trailing: const Icon(Icons.chevron_right), onTap: () => _open(context, const ReportsPage())),
    ListTile(leading: const Icon(Icons.settings, color: _lime), title: const Text('Settings'), trailing: const Icon(Icons.chevron_right), onTap: () => _open(context, const SettingsPage())),
  ]);
}
