...
class _DashboardPageState extends State<DashboardPage> {
  ...
  final _pages = const [
    _DashboardView(),
    _InventoryView(),
    _TransactionView(type: 'IN'),
    _TransactionView(type: 'OUT'),
    _MoreView(),
  ];
  ...
  Widget build(BuildContext context) {
    return ListView(
      ...
      children: [
        ...
        Text('Offline inventory control', style: Theme.of(context).textTheme.bodyMedium),
...
class _InventoryViewState extends State<_InventoryView> {
  ...
  void initState() {
    super.initState();
    _future = DatabaseService.inventory();
  }
          ...
          children: [
            ...
            if (rows.isEmpty)
              ...
              ...rows.map((row) => ListTile(
                    title: Text('${row['item_code']} • ${row['item_name']}'),
                    subtitle: Text('${row['uom']}'),
                    trailing: Text('${row['balance']}'),
                  )),
          ],
        );
      },
  ...
  Future<void> _save() async {
    ...
    try {
      await DatabaseService.insertTransaction(
        ...
        tranx: widget.type,
        receive: widget.type == 'IN' ? quantity : 0,
        issue: widget.type == 'OUT' ? quantity : 0,
        opening: 0,
        docNo: _doc.text.trim(),
        charging: _charging.text.trim(),
...
class _MoreView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ListView(
      children: const [
        ListTile(leading: Icon(Icons.receipt_long), title: Text('Bin Card')),
        ListTile(leading: Icon(Icons.fact_check), title: Text('Stock Take')),
        ListTile(leading: Icon(Icons.local_gas_station), title: Text('Fuel')),
        ListTile(leading: Icon(Icons.eco), title: Text('Fertilizer')),
