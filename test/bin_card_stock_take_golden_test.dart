...
void main() {
  group('Gold Excel parity - Bin Card', () {
    ...
    test('matches Gold Excel opening and running balance', () {
      final actual = BinCardEngine.buildForPeriod(
        itemCode: '010001',
  ...
  group('Stock Take parity contract', () {
    test('variance and status match the Excel rule', () {
      expect(
        StockTakeEngine.calculate(system: 100, physical: 100).variance,
        0,
      );
      expect(
        StockTakeEngine.calculate(system: 100, physical: 100).status,
        'TALLY',
      );
      expect(
        StockTakeEngine.calculate(system: 100, physical: 105).variance,
        5,
      );
      expect(
        StockTakeEngine.calculate(system: 100, physical: 105).status,
        'OVER',
      );
      expect(
        StockTakeEngine.calculate(system: 100, physical: 95).variance,
        -5,
      );
      expect(
        StockTakeEngine.calculate(system: 100, physical: 95).status,
        'SHORT',
      );
    });
    ...
    test('system balance follows native inventory contract', () {
      ...
      final system = InventoryEngine.stockTakeBalance(
        rows: rows,
        itemCode: '010001',
        period: DateTime(2026, 8, 1),
      );
